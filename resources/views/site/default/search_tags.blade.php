                        ...
                        <div class="row">
                            @foreach ($posts as $post)
                                <div class="col-md-6">
                                    <div class="item ">
                                        <div class="CropImg">
                                            <a href="{{ route('post', ['cate_slug' => ($languageCurrent == 'vn') ? 'tin-tuc' : 'news', 'post_slug' => $post->slug]) }}" class="thumbs">
                                                <img src="{{ asset($post->image) }}" title="{{ $post->title }}"  alt="{{ $post->title }}"/>
                                            </a>
                                        </div>
                                        <h2 class="title boxNewT">
                                            <a href="{{ route('post', ['cate_slug' => ($languageCurrent == 'vn') ? 'tin-tuc' : 'news', 'post_slug' => $post->slug]) }}">
                                                {{ $post->title }}
                                        ...
                                        <div class="readMore"><a href="{{ route('post', ['cate_slug' => ($languageCurrent == 'vn') ? 'tin-tuc' : 'news', 'post_slug' => $post->slug]) }}">
